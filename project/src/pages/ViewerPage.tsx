...
import type { Content, Classroom, AudioPlayerRef, ContentNavigation, HeaderInfo, SlideInfo } from '../types';
...
const AudioPlayer = forwardRef<AudioPlayerRef, AudioPlayerProps>(({ fileId, onPlayStateChange, onEnded, initialPlaybackRate = 1, onPlaybackRateChange, onLoadingChange }, ref) => {
...
interface SlideViewerProps {
  ...
  audioPlayerComponent: React.ReactNode;
...
const SlideViewer: React.FC<SlideViewerProps> = ({
  ...
  const showAudioBarWithTimeout = (): void => {
  ...
  const detectAndSetupSlides = (): void => {
    try {
  ...
  const hideContentNavigation = (): void => {
    try {
      const iframeDoc = iframeRef.current?.contentDocument || iframeRef.current?.contentWindow?.document;
      if (!iframeDoc) return;
  ...
  const goToSlide = (targetIndex: number): void => {
    try {
      ...
      try {
        iframeDoc.documentElement.scrollTop = 0;
        iframeDoc.body.scrollTop = 0;
        iframeRef.current!.contentWindow?.scrollTo({ top: 0, left: 0, behavior: 'instant' as ScrollBehavior });
      } catch (scrollError) {
  ...
  if (isFullscreen) {
    ...
    return (
      ...
      >
        ...
        <iframe
          ref={iframeRef}
          srcDoc={enhancedHtmlContent || undefined}
                ...
                {isAudioLoading ? (
                  ...
                  <>
                    <button
                      ...
                      onContextMenu={(e: React.MouseEvent) => { e.preventDefault(); showAudioBarWithTimeout(); }}
...
const ViewerPage: React.FC = () => {
  ...
  const [isAudioLoading, setIsAudioLoading] = useState<boolean>(true); // Track audio loading state
    ...
    const fetchData = async (): Promise<void> => {
      try {
        ...
        try {
          ...
          setClassroom({ id: contentData.classroomId, name: '教室' } as Classroom);
        }
  ...
  const handleCyclePlaybackRate = (): void => {
  ...
  const handleAudioSeek = (time: number): void => {
    audioPlayerRef.current?.seekTo(time);
  };

  const handleAudioSkip = (seconds: number): void => {
    audioPlayerRef.current?.skip(seconds);
  };

  const handlePlaybackRateChange = (newRate: number): void => {
    if (newRate !== undefined) {
      setPlaybackRate(newRate);
    }
  };

  const handleAudioPlayStateChange = (playing: boolean): void => {
    setIsAudioPlaying(playing);
  };

  const handleAudioLoadingChange = (isLoading: boolean): void => {
  ...
  const headerInfoProps: HeaderInfo = {
    title: content!.title,
        ...
        {hasNext && (
          <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer">
            <input
              type="checkbox"
              checked={autoNext}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => setAutoNext(e.target.checked)}
  ...
  return (
    <>
      <SlideViewer
        ...
        contentNavigation={contentNavigationProps}
      ...
      {/* RAG ChatPanel - ragEnabled classroom only */}
      {(classroom as Classroom & { ragEnabled?: boolean })?.ragEnabled && (
        <ChatPanel
