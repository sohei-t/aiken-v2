...
import type { Classroom, Content, WatchHistoryMap } from '../types';
...
const formatTotalDuration = (seconds: number): string | null => {
...
interface ClassroomListItemProps {
  classroom: Classroom & { depth?: number };
  progress?: ClassroomProgressData;
  childCount?: number;
  isExpanded?: boolean;
  onToggle?: () => void;
}

const ClassroomListItem: React.FC<ClassroomListItemProps> = ({ classroom, progress, childCount = 0, isExpanded, onToggle }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 flex items-center gap-4 text-sm text-gray-500">
        {progress?.totalDuration != null && progress.totalDuration > 0 && (
...
const TopPage: React.FC = () => {
  ...
  const [progressData, setProgressData] = useState<Record<string, ClassroomProgressData>>({});
  ...
  useEffect(() => {
    const fetchProgressData = async (): Promise<void> => {
      ...
      const progress: Record<string, ClassroomProgressData> = {};
  ...
  const { rootClassrooms, childClassroomsMap } = useMemo(() => {
    const classroomIdSet = new Set(classrooms.map(c => c.id));
    const roots: (Classroom & { depth: number })[] = [];
    const childMap: Record<string, Classroom[]> = {};
    classrooms.forEach(c => {
      if (!c.parentClassroomId || !classroomIdSet.has(c.parentClassroomId)) {
  ...
  const getFilteredChildren = (parentId: string): Classroom[] => {
