...
import { BookOpen, Users, BarChart3, FileText, Loader2, ArrowRight } from 'lucide-react';
...
const MemberProgressRow = ({ user, progress }) => {
  const p = progress || { watchedCount: 0, totalCount: 0, progressPercent: 0 };
  return (
...
const DashboardPage = () => {
  const { userData, customerId, isAdmin } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch progress in background (can be slow for large orgs)
  ...
  const avgProgress = members.length > 0
    ? Math.round(members.reduce((sum, m) => sum + (progress[m.id]?.progressPercent || 0), 0) / members.length)
    : 0;
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {members.map(user => (
                    <MemberProgressRow key={user.id} user={user} progress={progress[user.id]} />
                  ))}
                </tbody>
              </table>
            </div>
