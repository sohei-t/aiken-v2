...
const getBadge = (content) => {
  ...
  if (createdAt && (now - createdAt) < THREE_DAYS) {
    return { label: 'NEW!', bg: 'bg-red-100', text: 'text-red-600' };
  }
  const contentUpdatedAt = content.contentUpdatedAt?.toDate?.()
    || (content.contentUpdatedAt ? new Date(content.contentUpdatedAt) : null);
  if (contentUpdatedAt && (now - contentUpdatedAt) < SEVEN_DAYS) {
...
const ChildClassroomItem = ({ classroom }) => {
  return (
    <Link
...
const ClassroomPage = () => {
  ...
  const [sortBy, setSortBy] = useState('order'); // 'order', 'title', 'duration', 'createdAt'
