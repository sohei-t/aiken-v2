...
import { getDashboardStats, getCustomerUsers, getCustomer } from '../../services/firebase';
import { Loader2, BookOpen, FileText, Users, HardDrive, Sparkles, Crown, ArrowUpRight } from 'lucide-react';
...
const UsageBar = ({ label, icon: Icon, current, limit, unit = '' }) => {
...
const PlanUsage = () => {
  ...
  const planId = customerData?.plan || 'trial';
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        ...
        {planId === 'trial' && customerData?.trialExpiresAt && (
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700">
            トライアル期間: {new Date(customerData.trialExpiresAt.seconds * 1000).toLocaleDateString('ja-JP')} まで
          </div>
        )}
      </div>
