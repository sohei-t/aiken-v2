...
import type { CustomerSettings, StatusMessage } from '../../types';
...
const ApiKeyInput: React.FC<ApiKeyInputProps> = ({ label, description, value, onChange, placeholder }) => {
...
const ApiSettings: React.FC = () => {
  ...
  const handleSave = async (): Promise<void> => {
    ...
    setMessage(null);
    try {
      await updateCustomerSettings(customerId!, settings as CustomerSettings);
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            <ApiKeyInput
              ...
              onChange={(v: string) => setSettings(prev => ({ ...prev, claudeApiKey: v }))}
              placeholder="sk-ant-..."
            />
            <ApiKeyInput
              ...
              onChange={(v: string) => setSettings(prev => ({ ...prev, openaiApiKey: v }))}
              placeholder="sk-..."
            />
            <ApiKeyInput
              ...
              onChange={(v: string) => setSettings(prev => ({ ...prev, geminiApiKey: v }))}
              placeholder="AIza..."
            />
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSettings(prev => ({ ...prev, ttsProvider: e.target.value }))}
            ...
            {settings.ttsProvider === 'google' && (
              <ApiKeyInput
                ...
                onChange={(v: string) => setSettings(prev => ({ ...prev, googleTtsApiKey: v }))}
                placeholder="AIza..."
              />
            )}
          </div>
