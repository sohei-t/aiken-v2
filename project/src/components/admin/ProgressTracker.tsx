...
import type { Classroom, Content, UserData, ClassroomProgress, UserProgressMatrix } from '../../types';

const ProgressTracker: React.FC = () => {
  const { customerId } = useAuth();
  const [classrooms, setClassrooms] = useState<Classroom[]>([]);
  const [members, setMembers] = useState<UserData[]>([]);
  const [contentsByClassroom, setContentsByClassroom] = useState<Record<string, Content[]>>({});
  const [watchData, setWatchData] = useState<Record<string, Set<string>>>({}); // { userId: Set<contentId> }
  ...
  useEffect(() => {
    ...
    const fetchData = async (): Promise<void> => {
      try {
        ...
        const watchMap: Record<string, Set<string>> = {};
  ...
  const progressMatrix = useMemo((): UserProgressMatrix[] => {
    return members.map(user => {
      const watched = watchData[user.id] || new Set<string>();
      const classroomProgress: Record<string, ClassroomProgress> = {};
      let totalWatched = 0;
      let totalContents = 0;
  ...
  const filteredProgress = useMemo((): UserProgressMatrix[] => {
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <select
          ...
          onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSelectedClassroom(e.target.value)}
